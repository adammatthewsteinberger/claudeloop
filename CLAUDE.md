...
`autoclaude`: an onion-architected, autonomous Claude Code session runner
...
waitable — needs a human to top up). Pre-1.0; milestone M1 (the pure domain
core) is complete, M2–M5 are roadmap.
...
`autoclaude-architecture` skill before adding any new file.
