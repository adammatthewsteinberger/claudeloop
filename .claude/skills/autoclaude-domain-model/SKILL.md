...
# autoclaude domain model

Everything in `src/autoclaude/domain/` is a frozen dataclass or a closed
...
exact bug (`autoclaude` replacing sleeping-an-hour-forever with something
...
None`): substring-match `AUTOCLAUDE_TASK_FULLY_COMPLETE` (or the configured
